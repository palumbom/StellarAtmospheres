...
val_old = SA.dfv
val_new = SA.interp_valIIIc(npoints=200)
# val_new = SA.interp_valIIIc(val_old.h)
...
# plot it
plt.plot(h_old, temp_old, label="Old")
plt.plot(h, temp, ":", label="New")
plt.legend()
plt.show()
...
κ_na1_old = κ_line(λs, temp_old, Pe_old, Pg_old, ξ_old, nH_old, ρ_old, NaD1)
κ_na2_old = κ_line(λs, temp_old, Pe_old, Pg_old, ξ_old, nH_old, ρ_old, NaD2)
