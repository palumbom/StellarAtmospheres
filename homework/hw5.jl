...
fig.savefig(outdir * "hw5_F_nu_0_8777.pdf")
plt.clf(); plt.close()

# calculate & plot flux at values of τ
τ = [0.01, 0.1, 0.5, 0.67, 1.0, 2.0, 3.0, 10.0]
o = zeros(length(ν), length(τ))

fig = plt.figure()
ax1 = fig.add_subplot(111)
for i in eachindex(τ)
    lab = L"\tau=\ " * string(τ[i])
    o[:,i] = ℱντ(SνPlanck, τ[i], τs, Teff=T, ν=ν, ntrap=500)
    ax1.plot(ν̃, o[:,i], label=lab)
end
ax1.set_xlabel(L"\tilde{\nu}\ (\mu{\rm m}^{-1})")
...
ax1.legend(ncol=2)
fig.savefig(outdir * "hw5_F_nu_tau_8777.pdf")
plt.clf(); plt.close()

# integrate ℱντ over ν
τ = range(0.01, 10.01, length=10)
Fτs = similar(τ)
for i in eachindex(τ)
...
fig.savefig(outdir*"hw5_F_tau_8777.pdf")
plt.clf(); plt.close()
